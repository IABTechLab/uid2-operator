...
public class DomainNameCheckUtil {
    ...
    public static boolean isDomainNameAllowed(String origin, Set<String> allowedDomainNameSet) {
        ...
        try {
            ...
            LOGGER.info("isDomainNameAllowed Origin=" + origin + " is malformed URL. Rejecting.");
            return false;
        } catch (Exception e) {
            LOGGER.info("isDomainNameAllowed Origin" + origin + " not producing top level domain name correctly:." + e + " Rejecting.");
            return false;
        }

        if (topLevelDomainName == null) {
            LOGGER.info("isDomainNameAllowed Origin" + origin + " returns null top level domain name value. Rejecting.");
            return false;
        }
